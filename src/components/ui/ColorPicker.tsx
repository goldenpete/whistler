...
const PRESET_COLOR_LABELS: Record<string, string> = {
  "#f59e0b": "Amber",
  "#ef4444": "Red",
  "#f97316": "Orange",
  "#84cc16": "Lime",
...
export function ColorPicker({
  ...
  const getPresetColorLabel = (value: string) =>
    PRESET_COLOR_LABELS[value.trim().toLowerCase()] ?? value;
  ...
  return (
    <div className="space-y-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Select
            ...
            onValueChange={(val: string) =>
              setFormat(val as "hex" | "rgb" | "hsl")
            }
          >
            <SelectTrigger
              size="sm"
              className="h-8 bg-zinc-900 border-zinc-800 text-zinc-300"
            >
      ...
      <div className="flex flex-wrap justify-center gap-2 p-3 bg-zinc-900/50 rounded-none border border-white/5">
        {PRESET_COLORS.map((c) => (
          <AppTooltip key={c} content={getPresetColorLabel(c)}>
            <button
              type="button"
              onClick={() => {
                onChange(c);
              ...
              aria-label={getPresetColorLabel(c)}
            >
              {color.toLowerCase() === c.toLowerCase() && (
                <Check
                  weight="bold"
                  className={cn(
                    "w-3 h-3",
                    c === "#ffffff" ? "text-black" : "text-white shadow-sm",
                  )}
                />
              )}
            </button>
        ...
        <AppTooltip content="Custom Color">
          <button
            type="button"
          ...
          >
            <Palette
              weight="bold"
              className="w-3 h-3 text-zinc-400 group-hover:text-white"
            />
            <input
              type="color"
              value={color || "#000000"}
        ...
        <AppTooltip content="No Color">
          <button
            ...
            className={cn(
              "w-6 h-6 rounded-none transition-all border-2 relative flex items-center justify-center group",
              !color
                ? "border-white scale-110 bg-zinc-800"
                : "border-transparent hover:scale-110 hover:border-white/20 bg-zinc-800/50",
            )}
            aria-label="No Color"
          >
            {!color && (
