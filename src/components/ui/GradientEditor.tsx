...
export function GradientEditor({ value, onChange }: GradientEditorProps) {
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="space-y-2">
                <div className="h-4 w-full border border-border relative mb-6" style={{ background: `linear-gradient(90deg, ${stops.map(s => `${s.color || 'transparent'} ${s.position}%`).join(', ')})` }}>
                    {stops.map((stop) => (
                        <Popover key={stop.id}>
                            <PopoverTrigger asChild>
                                <button
                                    className="absolute w-3 h-3 -ml-1.5 top-1/2 -translate-y-1/2 border-2 border-white shadow-sm ring-1 ring-black/20 hover:scale-125 transition-transform focus:outline-none"
                                    style={{ 
                                        left: `${stop.position}%`,
