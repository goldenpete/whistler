...
import {
  ...
  NewGraphDialog,
  NewProjectDialog,
} from "@/components/dialogs/CreationDialogs";
...
const VideoCardPreview = ({
  url,
  start = 0.1,
  overrideMiddleFrame = false,
}: {
  url: string;
  start?: number;
  overrideMiddleFrame?: boolean;
}) => {
  const { useMiddleFrameForPreviews, cacheFiles } = useStore(
    useShallow((state) => ({
  ...
  ({ item }: { item: HomeViewItem }) => {
      ...
      return (
        <>
          ...
          <div
            ...
            style={
              item.data.color ? { color: item.data.color as string } : undefined
            }
          >
      ...
      return (
        <>
          ...
          <div
            ...
            style={
              item.data.color
                ? { color: item.data.color as string, opacity: 0.05 }
                : undefined
            }
          >
    ...
    if (item.type === "highlight") {
      ...
      if (
        file &&
        resolvedUrl &&
        (file.type === "video" || file.type === "image" || file.type === "pdf")
      ) {
        if (file.type === "image") {
          return (
            <div className="absolute inset-0 bg-black/20">
        ...
        if (file.type === "video") {
          return (
            <VideoCardPreview
              url={resolvedUrl}
              start={item.data.start || 0.1}
              overrideMiddleFrame={true}
            />
          );
        }
...
export default function HomeView() {
  ...
  const [quickAccessType, setQuickAccessType] =
    useState<QuickAccessType | null>(null);
  ...
  const handleColorChange = (file: AppFile, color: string) => {
    useStore.setState((state) => ({
      files: state.files.map((f: AppFile) =>
        f.id === file.id ? { ...f, color, lastModified: Date.now() } : f,
      ),
    }));
  };
  ...
  const handleIconChange = (file: AppFile, icon: string) => {
    useStore.setState((state) => ({
      files: state.files.map((f: AppFile) =>
        f.id === file.id ? { ...f, icon, lastModified: Date.now() } : f,
      ),
    }));
  };
  ...
  const handleAddFile = (url: string, name: string) => {
    ...
    if (!isValidUrl(trimmedUrl)) {
      alert(
        "Invalid URL. Only http, https, blob, and data protocols are allowed.",
      );
      return;
    }
    ...
    if (!targetStorageId) {
      const projectStorages = storages.filter(
    ...
    const newFile: AppFile = {
      ...
      type,
      order: files.filter(
        (f: AppFile) => f.projectId === activeProjectId && !f.parentId,
      ).length,
      created: Date.now(),
      lastModified: Date.now(),
    };
  ...
  const handleAddCloudFile = (draft: CloudFileDraft) => {
    ...
    if (!cloudSource) {
      alert(
        `Unsupported ${getCloudProviderLabel(draft.provider)} link. Use a public file share URL.`,
      );
      return;
    }
  ...
  const handleCreateCollection = (
    ...
    if (!targetBucketId) {
      const firstBucket = collections.find(
        (c: Collection) =>
    ...
    const sameParentItems = collections.filter(
      (c) => c.parentId === targetBucketId,
    );
    const maxOrder =
      sameParentItems.length > 0
  ...
  const handleCreateBucket = (name: string, color: string, icon: string) => {
    ...
    const siblingBuckets = collections.filter(
      (c: Collection) =>
        c.projectId === activeProjectId &&
        c.parentId === null &&
        c.type === "bucket" &&
  ...
  const projectFiles = useMemo(
    () =>
      files.filter(
        (f: AppFile) => f.projectId === activeProjectId && !f.deleted,
      ),
  ...
  const projectGraphs = useMemo(
    () => (graphs || []).filter((g) => g.projectId === activeProjectId),
    [graphs, activeProjectId],
  );

  // Recent Highlights logic
  const projectFileIds = useMemo(
    () => new Set(projectFiles.map((f: AppFile) => f.id)),
    [projectFiles],
  );
  const projectHighlights = useMemo(
    () => highlights.filter((h) => projectFileIds.has(h.fileId)),
    [highlights, projectFileIds],
  );

  const formatHighlightLabel = (h: Highlight, file?: AppFile) => {
    if (file?.type === "pdf") {
      return h.end && h.end !== h.start
        ? `Page ${h.start}-${h.end}`
        : `Page ${h.start}`;
    }
        ...
        ...storages
          ...
          .map((s) => ({
            ...
            timestamp: Math.max(
              ...
              (s as Storage & { lastViewed?: number }).lastViewed || 0,
            ),
            data: s,
          })),
        ...(projects || []).map((p) => ({
          ...
          name: p.name,
          timestamp: Math.max(
            p.lastModified || p.created,
            (p as Project & { lastViewed?: number }).lastViewed || 0,
          ),
          data: p,
        })),
  ...
  const filteredItems = useMemo(
    () => allItems.filter((item) => activeFilters.has(item.type)),
  ...
  const getItemIcon = (item: (typeof allItems)[0]) => {
    ...
    switch (item.type) {
      case "file":
        return getFileIcon(item.subType as AppFile["type"]);
      case "doc":
        return FileText;
      case "collection":
        return Tag;
  ...
  const handleItemClick = (item: (typeof allItems)[0]) => {
    switch (item.type) {
      ...
      case "collection":
        {
          const bucketId = findRootBucketId(
    ...
    switch (type) {
      case "file":
        ...
        if (isCloudFile(renameItem.data)) {
          ...
          const provider =
            detectCloudProvider(nextUrl) ||
            renameItem.data.cloudSource.provider;
          const cloudSource = createCloudFileSource(provider, nextUrl);
          if (!cloudSource) {
            alert(
              "Unsupported cloud share link. Use a public Google Drive, Dropbox, or OneDrive file link.",
            );
            return;
          }
  ...
  return (
    <div className="flex flex-col h-full bg-transparent overflow-hidden">
      ...
      <div className="flex items-center gap-4 p-8 pb-4 shrink-0">
        ...
        <Popover open={filterPopoverOpen} onOpenChange={setFilterPopoverOpen}>
          ...
          <PopoverContent className="w-56 p-1" align="end">
            ...
            {[
              ...
              >
                <div
                  className={`flex items-center justify-center w-4 h-4 rounded-sm border ${activeFilters.has(type) ? "bg-primary border-primary text-primary-foreground" : "border-muted-foreground opacity-50"}`}
                >
                  {activeFilters.has(type) && <Check size={10} weight="bold" />}
                </div>
                <span className="capitalize">
                  {type === "file" ? "Files" : type + "s"}
                </span>
              </Button>
            ))}
          </PopoverContent>
        ...
        <Popover
          open={quickAccessPopoverOpen}
          onOpenChange={setQuickAccessPopoverOpen}
        >
          <PopoverTrigger asChild>
            <Button variant="outline" className="gap-2">
          ...
          <PopoverContent className="w-48 p-1" align="end">
            ...
            <Button
              ...
              className="w-full justify-start gap-2 h-9 px-2 font-normal"
              onClick={() => {
                setQuickAccessType("storage");
                setQuickAccessOpen(true);
                setQuickAccessPopoverOpen(false);
              }}
            >
              <HardDrives className="text-muted-foreground" size={16} />{" "}
              Storages
            </Button>
        ...
        <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
          ...
          <PopoverContent className="w-48 p-1" align="end">
            ...
            >
              <GraphIcon className="text-muted-foreground" size={16} /> Add
              Graph
            </Button>
            <Button
              variant="ghost"
            ...
            >
              <HardDrives className="text-muted-foreground" size={16} /> Add
              Storage
            </Button>
            <div className="h-px bg-border my-1" />
            <Button
      ...
      {/* Content Grid */}
      <div
        className="flex-1 overflow-auto p-8 pt-4"
                  ...
                  <ContextMenu key={`${item.type}-${item.id}`}>
                    <ContextMenuTrigger asChild>
                      <button
                        ...
                        style={{
                          ...
                          containIntrinsicBlockSize: "12rem",
                        }}
                      >
                        {/* Background Preview */}
                        ...
                        <div className="relative z-20 w-full flex flex-col h-full">
                          ...
                          <div className="w-full min-w-0 flex flex-col gap-1 mt-4">
                            <div
                              className="font-semibold text-sm truncate leading-tight group-hover:text-primary transition-colors"
                              title={item.name}
                            >
                              {item.name}
                            </div>
                    ...
                    {item.type === "file" ? (
                      <FileContextMenu
                        ...
                        onSelect={() => {}}
                        onColorChange={(color) =>
                          handleColorChange(item.data, color)
                        }
                        onIconChange={(icon) =>
                          handleIconChange(item.data, icon)
                        }
                      />
                    ) : (
                      <ContextMenuContent>
                        ...
                        {item.type === "highlight" ? (
                          <>
                            <ContextMenuItem
                              onClick={() =>
                                handleCopy(item.data.text || item.data.note)
                              }
                            >
      ...
      {renameItem && (
        <>
          <RenameFileDialog
            ...
            onSubmit={(name, description, url, color) =>
              handleRename(name, description, color, undefined, url)
            }
            initialName={renameItem.name}
            initialDescription={
              renameItem.type === "file"
                ? renameItem.data.description
                : undefined
            }
            initialUrl={
              renameItem.type === "file"
                ? isCloudFile(renameItem.data)
                  ? renameItem.data.cloudSource.shareUrl
                  : renameItem.data.url || undefined
                : undefined
            }
            initialColor={
              renameItem.type === "file" ? renameItem.data.color : undefined
            }
            isLocalFileSource={
              renameItem.type === "file" ? isLocalFile(renameItem.data) : false
            }
            localSourceLabel={
              renameItem.type === "file"
                ? getDisplaySourceLabel(renameItem.data)
                : ""
            }
          />
          <EditDocDialog
            ...
            onSubmit={(name, color, icon) =>
              handleRename(name, undefined, color, icon)
            }
            initialName={renameItem.name}
            initialColor={
              renameItem.type === "doc" ? renameItem.data.color : undefined
            }
            initialIcon={
              renameItem.type === "doc" ? renameItem.data.icon : undefined
            }
          />
          <EditFolderDialog
            ...
            initialName={renameItem.name}
            initialColor={
              renameItem.type === "collection" || renameItem.type === "bucket"
                ? renameItem.data.color
                : undefined
            }
            initialIcon={
              renameItem.type === "collection" || renameItem.type === "bucket"
                ? renameItem.data.icon
                : undefined
            }
          />
          <EditGraphDialog
            ...
            initialName={renameItem.name}
            initialColor={
              renameItem.type === "graph" ? renameItem.data.color : undefined
            }
            initialIcon={
              renameItem.type === "graph" ? renameItem.data.icon : undefined
            }
          />
          <MoveFileDialog
            open={moveDialogOpen}
