...
export type CloudFileTypeSelection =
  | "auto"
...
export function isCloudFile(
  file: AppFile | null | undefined,
): file is AppFile & { sourceKind: "cloud"; cloudSource: CloudFileSource } {
  return Boolean(
    file?.sourceKind === "cloud" &&
    file.cloudSource?.provider &&
    file.cloudSource?.shareUrl &&
    file.cloudSource?.directUrl,
  );
}
...
export function resolveCloudFileUrl(
  ...
  try {
    ...
    const freshUrl = resolveCloudDirectUrl(
    ...
    if (
      cloudSource.provider === "google-drive" &&
      !options.allowGoogleDriveLegacyFallback
    ) {
      return null;
    }
 ...
 */
export function getGoogleDriveEmbedUrl(
  cloudSource: CloudFileSource,
): string | null {
...
export function detectCloudProvider(url: string): CloudProvider | null {
  try {
    ...
    if (
      ...
      host.includes("dropboxusercontent.com")
    ) {
      return "dropbox";
    }
...
export function createCloudFileSource(
  provider: CloudProvider,
  shareUrl: string,
): CloudFileSource | null {
...
export function inferCloudFileType(
  ...
  return fallback === "folder" ? "file" : fallback;
}

function resolveCloudDirectUrl(
  provider: CloudProvider,
  parsed: URL,
  options: ResolveCloudFileUrlOptions = {},
): string | null {
  switch (provider) {
    case "google-drive": {
      return buildGoogleDriveDirectUrl(
        parsed,
        options.googleDriveApiKey,
        options.allowGoogleDriveLegacyFallback,
      );
    }
    case "dropbox": {
...
function buildGoogleDriveDirectUrl(
  parsed: URL,
  googleDriveApiKey?: string | null,
...
export function extractGoogleDriveFileId(parsed: URL): string | null {
  ...
  const documentMatch = pathname.match(
    /\/(?:document|presentation|spreadsheets)\/d\/([^/]+)/i,
  );
  if (documentMatch?.[1]) {
